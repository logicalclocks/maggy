...
def distributed_lagom(train_fn, name, hb_interval, description):
    global RUNNING
    if RUNNING:
    ...
    try:
        ...
        APP_ID, RUN_ID, RUNNING = register_environment(APP_ID, RUN_ID, RUNNING, spark_context)
        num_executors = util.num_executors(spark_context)
        exp_driver = DistributedDriver(name=name, description=description,
        ...
        spark_context.setJobGroup(os.environ["ML_ID"], "{0} | distributed_learning".format(name))
        EXPERIMENT_JSON = experiment_utils._populate_experiment(name, "distributed_learning",
                                                                "MAGGY", None, description, APP_ID,
                                                                "max", "metric")
        exp_ml_id = APP_ID + "_" + str(RUN_ID)
        EXPERIMENT_JSON = experiment_utils._attach_experiment_xattr(exp_ml_id, EXPERIMENT_JSON,
                                                                    "INIT")
        ...
        util._log("Started Maggy Experiment: {0}, {1}, run {2}".format(name, APP_ID, RUN_ID))
        ...
        _exception_handler(experiment_utils._seconds_to_milliseconds(time.time() - job_start))
